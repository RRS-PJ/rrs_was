...
/**
 * JwtProvider 클래스
 * : WT(JSON Web Token) 토큰을 생성하고 검증하는 역할
 *
 * cf) JWT
 *      : 사용자 정보를 암호화된 토큰으로 저장, 서버에 매번 요헝할 때 전달 가능
 *      : 주로 로그인 징으에 사용
 *
...
public class JwtProvider {
    ...
    @Value("${jwt.expiration}")//(환경변수에 정의된 것을 가져와서?)
    ...
    public JwtProvider(@Value("${jwt.secret}") String secret, @Value("${jwt.expiration}") int jwtExpirationMs) {
        // 생성자: JWTProvider 객체를 생성할 때 비밀키와 만료 시간을 클래스 필드에 대한 초기화
    ...
    public Claims getClaims(String token) {
        ...
        // 파싱 : 주어진 데이터를 해석하고 분석하여
        // ,  원하는 형식 또는 구조로 변환하는 작업
        return jwtParser.parseClaimsJws(token).getBody();
        // 기능을 분리하기위해 따로 작성하는 경우
    }
}